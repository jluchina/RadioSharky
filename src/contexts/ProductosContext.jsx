...
function obtenerProductos() {
  return new Promise((res, rej) => {
    if (productosCargados) {
      return res(productos); // Ya están cargados
    }
    ...
    const agregarProducto = (producto) => {
        return(
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch('https://68100d8e27f2fdac24101f50.mockapi.io/productos', {
                        method: 'POST',
                        headers: {
    ...
    function editarProducto(producto){
        return(
            new Promise(async(res, rej) => {
            try {
                const respuesta = await fetch(`https://68100d8e27f2fdac24101f50.mockapi.io/productos/${producto.id}`, {
                    method: 'PUT',
                    headers: {
                    'Content-Type': 'application/json',
                    },
    ...
    const eliminarProducto = (id) => {
        ...
        if (confirmar) {
            return(
                new Promise(async (res, rej) => {
                    try {
                        const respuesta = await fetch(`https://68100d8e27f2fdac24101f50.mockapi.io/productos/${id}`, {
                        method: 'DELETE',
                        });
